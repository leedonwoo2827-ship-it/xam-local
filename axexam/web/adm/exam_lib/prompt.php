 ...
 * 초안 1건의 출력 상한과 대기 시간.
 *
 * ── 왜 1200 이 아닌가 (2026-08-04 실측) ────────────────────────────────────
 * 1200 으로 5건을 돌렸더니 3건이 빈 응답으로 실패했다. 진단을 붙여 보니:
 *
 *   finish_reason=length · 출력 토큰 1200 · 사고과정 1832자
 *
 * `deepseek-v4-flash` 는 **추론형 모델**이다. 1200 토큰을 사고 과정
 * (`reasoning_content`)에 전부 쓰고 본문(`content`)에 쓸 것이 남지 않았다.
 * 같은 모델로 2건이 성공한 것은 그 문항의 사고가 짧았기 때문이고 —
 * 그래서 "모델명이 틀렸다" 로 보였다.
 *
 * ★ 사고 과정은 **답이 아니라 사고**다. 그걸 답변으로 쓰면 안 된다.
 *   해결은 상한을 올려 본문이 나올 자리를 만드는 것이다.
 *
 * 2400 으로 정한 근거: 사고가 1200 토큰 정도 나가고 답변은 3~6문장(≈400 토큰)이면
 * 되므로 두 배가 여유 있다. 원가는 실측 0.81원/건 → 최악 약 1.6원/건이고
 * `ex_product.cost_cap` 3원 안쪽이다. 더 올리면 사고가 더 길어질 뿐이라 이득이 없다.
 *
 * 시간은 60 → 90 초. 출력이 두 배면 생성 시간도 늘어난다. 이 값이 커지면
 * 일괄 처리의 최악 대기 시간(= 예산 + 이 값)도 같이 커진다 —
 * exam_qna_list.php 의 진행 표시가 같은 숫자를 쓴다.
 */
if (!defined('EX_DRAFT_MAX_TOKENS')) define('EX_DRAFT_MAX_TOKENS', 2400);
if (!defined('EX_DRAFT_TIMEOUT'))    define('EX_DRAFT_TIMEOUT', 90);
...
function ex_prompt_build($q, $prob)
{
    ...
    $sys = "당신은 {$pd_name} 자격증 강사다. 수험생 질문에 답하는 초안을 쓴다.\n"
         ...
         /* ★ 서식을 좁게 못 박는다. 우리 렌더러(exam/lib/md.php)가 아는 것만 쓰게 해야
            한다 — 모르는 문법을 쓰면 화면에 기호가 그대로 보인다(`|표|`, `### 제목`).
            지원 범위를 넓히는 것보다 출력을 좁히는 것이 안전하고 싸다. */
         . "8. 서식은 **굵게**, `코드`, '- 불릿' 세 가지만 쓴다.\n"
         . "   표·제목(#)·링크·이미지는 쓰지 않는다. 화면이 그 기호를 그대로 보여준다.\n"
         /* ★ 추론형 모델이 사고에 출력 토큰을 다 써서 본문이 빈 채로 잘리는 일이 있었다
            (finish_reason=length · 사고과정 1832자). 상한을 올려 자리를 만들었지만,
            길게 생각할 이유가 없다는 것을 프롬프트에서도 말해 준다 — 그러면 원가도 준다.
            정답과 해설을 이미 줬으므로 **다시 풀 필요가 없다**는 점이 근거다. */
         . "9. 정답과 해설이 이미 주어졌으므로 **다시 풀지 않는다.** 길게 추론하지 말고\n"
         . "   바로 설명을 쓴다.\n";
...
function ex_draft_one($qa_id, $admin_id = '')
{
    ...
    $r = ex_llm_call($q['pd_id'], ex_prompt_build($q, $prob),
                     EX_DRAFT_MAX_TOKENS, EX_DRAFT_TIMEOUT);
