...
/* 초안 일괄의 벽시계 예산(초). 화면의 진행 표시도 이 값으로 "얼마나 기다리면 되는지"를
   말하므로 상수를 한 곳에 둔다 — 두 곳에 적으면 안내와 실제가 갈린다. */
$DRAFT_BUDGET = 100;

/* ★ `drafting` 에서 멈춘 행을 풀 수 있게 한다.
 *
 *   ex_draft_one() 은 pending|draft_ready 를 원자적으로 `drafting` 으로 집는다.
 *   LLM 이 실패하면 스스로 pending 으로 되돌리지만, **브라우저를 닫거나 PHP 가
 *   중간에 죽으면 그 행은 영원히 drafting 에 남는다.** 그 상태는 다시 집히지
 *   않으므로 그 질문은 큐에서 조용히 사라진다 — 가장 찾기 어려운 종류의 고장이다.
 *   (100초 예산이 있어도 예산 검사 뒤에 시작한 1건은 최대 60초를 더 쓴다.)
 *
 *   시간으로 판정한다: 10분이 지난 drafting 은 살아 있는 요청이 아니다.
 *   LLM 호출 타임아웃이 60초라 정상 처리가 10분을 넘길 수 없다.
 */
$STUCK_MIN = 10;
$stuck_n = 0;
