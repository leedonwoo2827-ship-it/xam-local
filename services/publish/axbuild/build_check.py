...
# ★ 내장(vendor) 시 고친 단 한 줄 (2026-08-03).
#
#   원본에서 이 파일은 axexam 저장소의 `scripts/` 에 있었고 `parents[1]` 이 곧 그
#   저장소 루트였다. XAM LOCAL 안으로 들여오면서 위치가
#   `services/publish/axbuild/` 로 바뀌었고, `parents[1]` 은 `services/publish` 가
#   된다. 그러면 `data/youtube_map.<pd>.json` 과 `data/brand.json` 을 엉뚱한 곳에서
#   찾는다(조용히 "영상 없음" 으로 빌드된다).
#   → XAM LOCAL 저장소 루트를 가리키게 한다. 그 아래 `data/` 가 이 앱의 런타임
#     폴더이고, README 의 `--youtube-map data\youtube_map.bigdata.json` 이 가리키는
#     곳과도 같아진다. 템플릿은 `Path(__file__).with_name(...)` 이라 그대로 산다.
