    ...
    return {
        ...
        "properties": {
            ...
            "answer_index": {"type": "integer", "minimum": 0,
                             "maximum": N_CHOICES - 1},
            # ★ 양쪽 다 막아야 한다. 실측 2회로 확인한 것:
            #     기준(검증된 m01)      화면 166자 · 낭독 356자 → 편당 15분
            #     1차: 비율만 줬다      화면 919자 · 낭독 1552자 → 편당 47분
            #     2차: 상한만 걸었다    화면 147자 · 낭독  272자 → 편당 8.3분
            #   모델은 제시 구간의 **아래쪽에 붙는다.** 그래서 하한도 스키마로 강제한다.
            #   말로만 적으면 양쪽 다 넘긴다 — 두 번 다 그랬다.
            "explanation": {"type": "string", "minLength": 140, "maxLength": 320},
            "explanation_speech": {"type": "string", "minLength": 300, "maxLength": 520},
            # ── 그림 ────────────────────────────────────────────────────────
